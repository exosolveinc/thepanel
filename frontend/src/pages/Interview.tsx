...
import { RotateCcw, Focus, BookOpen, Layers, Mic, BarChart2, Code2, UserCheck, MessageSquare, FolderOpen } from 'lucide-react'
...
import HMVoicePanel from '../components/HMVoicePanel'
...
type View = 'main' | 'live' | 'deep' | 'arch' | 'behavioral' | 'technical' | 'code-practice' | 'hmv' | 'hmt' | 'docs'
...
const TABS: { id: View; label: string; icon: React.ReactNode; active: string; hover: string }[] = [
  ...
  { id: 'docs',          label: 'Docs',            icon: <FolderOpen size={10} />,       active: 'bg-violet-600/25 text-violet-300',   hover: 'hover:text-violet-400' },
]
...
export default function Interview({ onReset }: InterviewProps) {
  ...
  // Abort any in-flight drill request when the page unmounts.
  useEffect(() => () => { drillAbortRef.current?.abort() }, [])
            ...
            >
              <div className="shrink-0 border-b border-zinc-800/60 px-4 py-3">
                <QueryBar
                  ...
                  onPrefillConsumed={() => setFollowUpPrefill(undefined)}
                  compact
                />
              </div>
              <div className="flex-1 min-h-0 overflow-hidden">
                <AnswersPanel onFollowUp={q => { setFollowUpPrefill(q); setView('main') }} />
              </div>
            </div>
